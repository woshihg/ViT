...
class AdaptiveAttention(nn.Module):
    """
    一个包含自适应 token 修剪机制的 Transformer Encoder 层 (AViT 风格)。
    修剪决策基于 patch token 与 [CLS] token 的相似度。
    """
    ...
    def forward(self, x):
        ...
        # 3. 计算相似度分数 (CLS token 与 patch tokens 的点积)
        # cls_token: [B, 1, D], patch_tokens: [B, N, D]
        # -> scores: [B, N, 1]
        scores = torch.matmul(patch_tokens, cls_token.transpose(-1, -2))
...
class AdaptiveVisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        # 创建一个包含多个 AdaptiveAttention 层的列表
        self.transformer_layers = nn.ModuleList([
            AdaptiveAttention(embed_dim, num_heads, mlp_dim, dropout, keep_rate)
